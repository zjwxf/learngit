...
def maintask():
    ...
    for i in data_pool.iloc[0:]:
        ...
        if df["close"].iloc[0] > df['close'].iloc[1:21].max():  #df的date已经倒序今天为0，昨天为1， 0，1，2 今昨前
            k=1
            for k in range(1,21):
                df1_20_max=df["close"].iloc[k+1:k+21].max()
                ...
                #print(df.head())
                print(j,"Buy Buy Buy",'正在获取第%d家，股票代码%s.' % (j, i),"today close = ",df["close"].iloc[0],  "//20 day max =", df['close'].iloc[1:21].max())
    return 
